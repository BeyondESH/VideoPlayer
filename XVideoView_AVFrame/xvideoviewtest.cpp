    ...
    , ui(new Ui::XVideoViewTest),_frame(nullptr),_sdl_width(0),_sdl_height(0),_pix_size(0),_isStop(false),_file_size(0),_multiple(1)
{
    ...
    //打开yuv文件
    _yuv_file.open("C:/Users/19896/Videos/lucy.yuv",std::ios::binary);
    if(_yuv_file.is_open()!=true){
        qDebug()<<"打开yuv视频失败";
        return;
    }
    //获取文件大小
    _yuv_file.seekg(0,std::ios::end);
    _file_size=_yuv_file.tellg();
    _yuv_file.seekg(0,std::ios::beg);
    //视频文件尺寸
